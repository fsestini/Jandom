...
class RationalHalfMatrixSpecification extends AbstractOctagonSpecification {

  type A = RationalExt
  type B = BoxRationalDomain
  type M[S,X] = DBM[HalfMatrix, S, X]
  ...
  def e =
    (new DBMInstance[HalfMatrix]()(HalfMatrixMatrixInstance.halfMatrixIsMatrix)).funDBM
